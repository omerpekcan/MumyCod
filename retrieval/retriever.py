...
class CodeRetriever:
    def __init__(self, brain_path=None):
        if isinstance(brain_path, dict) or brain_path is None:
            self.brain_path = "memory/brain.json"
        else:
    ...
    def retrieve_relevant_chunks(self, query: str, top_k: int = 3) -> list:
        ...
        for item in self.data:
            ...
            chunk_text = item.get("text") or item.get("chunk") or item.get("chunk_text") or ""
